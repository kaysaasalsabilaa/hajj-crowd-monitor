...
def _draw_tracks(frame, confirmed_tracks, slow_ids=None, crowd_top_y=270):
    slow_ids = slow_ids or set()
    border_tracks = 0
    h, w = frame.shape[:2]
    border_margin = 15
...
class AnnotatedVideoWriter:
    ...
    def write_frame(self, frame, confirmed_tracks, frame_idx: int,
                    timestamp: float, slow_ids=None, n_ghost: int = 0):
        if not self._ok:
            return

        annotated = frame.copy()
        count     = len(confirmed_tracks)
        n_slow    = len(slow_ids) if slow_ids else 0

        if self.show_zone_line:
            _draw_background_zone_line(annotated, self.crowd_top_y)

        n_border = _draw_tracks(
            annotated, confirmed_tracks, slow_ids, self.crowd_top_y)
